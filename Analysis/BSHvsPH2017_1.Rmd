...
percentSatisfied <- function(qn, i) {
  ...
  vg_bsh <- sum(as.integer(summary(as.factor(bsh_a))[c("c. Neutral", "d. Poor", "e. Very poor")]), na.rm = TRUE)
  g_bsh <- sum(as.integer(summary(as.factor(bsh_a))[c("a. Very good", "b. Good")]), na.rm = TRUE)
  tot_bsh <- length(nonbsh_a) - length(bsh_a[is.na(bsh_a)])
  percent_y_bsh <- (g_bsh)* 100 / (vg_bsh + g_bsh)
  
  vg_nonbsh <- sum(as.integer(summary(as.factor(nonbsh_a))[c("c. Neutral", "d. Poor", "e. Very poor")]), na.rm = TRUE)
  g_nonbsh <- sum(as.integer(summary(as.factor(nonbsh_a))[c("a. Very good", "b. Good")]), na.rm = TRUE)
  tot_nonbsh <- length(nonbsh_a) - length(nonbsh_a[is.na(nonbsh_a)])
  percent_y_nonbsh <- (g_nonbsh)* 100 / (vg_nonbsh + g_nonbsh)
...
ggplot(d, aes(reorder(Question, Percent_Satisfied), Percent_Satisfied, group=BSH_NonBSH)) + geom_bar(stat = "identity", width = 0.7, position="dodge", aes(fill=BSH_NonBSH), alpha=1) +
  ...
  geom_text(aes(y = Percent_Satisfied, label=Percent_Yes_text), stat="identity", position=position_dodge(width = 0.9), hjust="inward", vjust=.5, size=4, alpha=1) +
  scale_fill_manual(values = c("aquamarine3", "aquamarine4"), labels = c("pct_NO_bsh", "pct_NO_nonbsh"), name="")
