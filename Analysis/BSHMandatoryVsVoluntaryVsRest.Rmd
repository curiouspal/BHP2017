...
all1 <- subset(all, F!="Voluntary BSH") #!is.na(Member.Age...as.of.date_15))
...
Attr_15Median <- median(all1$Member.Age...as.of.date15[all1$F=="Mandatory BSH"], na.rm = TRUE)
restPH_15Median <- median(all1$Member.Age...as.of.date15[all1$F=="Non-BSH"], na.rm = TRUE)
med <- data.frame(F=c("Mandatory BSH", "Non-BSH"), m=c(Attr_15Median, restPH_15Median))
all1$med_15[all1$F=="Mandatory BSH"] <- Attr_15Median
all1$med_15[all1$F=="Non-BSH"] <- restPH_15Median
ggplot(all1, aes(x = all1$F, y = all1$Member.Age...as.of.date15)) + 
  ...
  ggtitle("Comparison of Age as of 2015 \nfor \"out\" group (N=29) and the restPH (N=88)\n")
all1 <- subset(all, F!="Mandatory BSH")
New17Median <- median(all1$'MemberAge-asofdate_17'[all1$F=="Voluntary BSH"], na.rm = TRUE)
restPH17Median <- median(all1$'MemberAge-asofdate_17'[all1$F=="Non-BSH"], na.rm = TRUE)
all1$med17[all1$F=="Voluntary BSH"] <- New17Median
all1$med17[all1$F=="Non-BSH"] <- restPH17Median
med <- data.frame(F=c("Voluntary BSH", "Non-BSH"), m=c(New17Median, restPH17Median))
...
#all1$'MemberAge-asofdate_17'[all1$F=="Non-BSH"]
#all1$'MemberAge-asofdate_17'[all1$F=="Voluntary BSH"]
```
...
all1 <- subset(all, F!="Voluntary BSH" & !is.na(Total.Annual.Income15))
att_2015_N <- length(all1$Total.Annual.Income15[all1$F=="Mandatory BSH"])
restPH_2015_N <- length(all1$Total.Annual.Income15[all1$F=="Non-BSH"])
Attr15Median <- median(all1$Total.Annual.Income15[all1$F=="Mandatory BSH"])
restPH15Median <- median(all1$Total.Annual.Income15[all1$F=="Non-BSH"])
med <- data.frame(F=c("Mandatory BSH", "Non-BSH"), m=c(Attr15Median, restPH15Median))
all1$med15[all1$F=="Mandatory BSH"] <- Attr15Median
all1$med15[all1$F=="Non-BSH"] <- restPH15Median
ggplot(all1, aes(x = all1$F, y = all1$Total.Annual.Income15)) + 
  geom_boxplot() +
  xlab("2015") + ylab("Total annual income in 2015") +
  geom_text(data = med, aes(x=med$F, y=med$m, label=med$m, vjust = -0.5, hjust=0.5, color = "red")) + theme(legend.position="none") +
  ggtitle(paste0("Comparison of Income as of 2015 \nfor \"out\" group (N=", att_2015_N, ") and the \"core\" (N=", restPH_2015_N, ")\n", sep = ""))

all1 <- subset(all, F!="Mandatory BSH")
att_2017_N <- length(all1$TotalAnnualIncome_17[all1$F=="Voluntary BSH"])
restPH_2017_N <- length(all1$TotalAnnualIncome_17[all1$F=="Non-BSH"])
#all1$TotalAnnualIncome_17 <- gsub(",", "", all1$TotalAnnualIncome_17)
#all1$TotalAnnualIncome_17 <- as.numeric(all1$TotalAnnualIncome_17)
New17Median <- median(all1$TotalAnnualIncome_17[all1$F=="Voluntary BSH"])
restPH17Median <- median(all1$TotalAnnualIncome_17[all1$F=="Non-BSH"])
all1$med17[all1$F=="Voluntary BSH"] <- New17Median
all1$med17[all1$F=="Non-BSH"] <- restPH17Median
med <- data.frame(F=c("Voluntary BSH", "Non-BSH"), m=c(New17Median, restPH17Median))
ggplot(all1, aes(x = all1$F, y = all1$TotalAnnualIncome_17)) + 
  ...
  geom_text(data = med, aes(x=med$F, y=med$m, label=med$m, vjust = -0.5, hjust=0.5, color = "red")) + theme(legend.position="none") +
  ggtitle(paste0("Comparison of Income as of 2017 \nfor \"out\" group (N=", att_2017_N, ") and the \"core\" (N=", restPH_2017_N, ")\n", sep = ""))
...
#diff<-restPH$Total.Annual.Income17-restPH$Total.Annual.Income14
#temp <- data.frame(restPH$TCode, restPH$Total.Annual.Income14, restPH$Total.Annual.Income17, diff)
#pander(temp)
#summary(temp$diff)
#temp <- subset(temp, restPH.Total.Annual.Income14!=0)
#summary(temp$diff)
