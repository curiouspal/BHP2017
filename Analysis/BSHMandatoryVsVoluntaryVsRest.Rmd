...
x <- eval(parse(text = x))
temp$N[1] <- length(x[!is.na(x)])
temp$PercentYes[1] <- 100*((summary(as.factor(x))[1] + summary(as.factor(x))[2])/(length(as.factor(x)[!is.na(as.factor(x))])))
...
all <- subset(all, F!="Voluntary BSH" & !is.na(Total.Annual.Income17))
att_2017_N <- length(all$Total.Annual.Income17[all$F=="Mandatory BSH"])
restPH_2017_N <- length(all$Total.Annual.Income17[all$F=="Non-BSH"])
Attr17Median <- median(all$Total.Annual.Income17[all$F=="Mandatory BSH"])
restPH17Median <- median(all$Total.Annual.Income17[all$F=="Non-BSH"])
med <- data.frame(F=c("Mandatory BSH", "Non-BSH"), m=c(Attr17Median, restPH17Median))
all$med17[all$F=="Mandatory BSH"] <- Attr17Median
all$med17[all$F=="Non-BSH"] <- restPH17Median
ggplot(all, aes(x = all$F, y = all$Total.Annual.Income17)) + 
  geom_boxplot() +
...
all <- subset(all, F!="Mandatory BSH")
att_2017_N <- length(all$TotalAnnualIncome_17[all$F=="Voluntary BSH"])
restPH_2017_N <- length(all$TotalAnnualIncome_17[all$F=="Non-BSH"])
#all$TotalAnnualIncome_17 <- gsub(",", "", all$TotalAnnualIncome_17)
#all$TotalAnnualIncome_17 <- as.numeric(all$TotalAnnualIncome_17)
...
all$med17[all$F=="Voluntary BSH"] <- voluntary17Median
all$med17[all$F=="Non-BSH"] <- restPH17Median
med <- data.frame(F=c("Voluntary BSH", "Non-BSH"), m=c(voluntary17Median, restPH17Median))
