...
#######################################################################################################
## this index would measure for each subquestion of (only) food domain, if a family moved between 2015 and 2017 from “very difficult “ to something 
## better (including “somewhat difficult”) or from “somewhat difficult” to something better.  Then add up the results for all 
## subquestions for the food domain. This variation of Method 1 would indicate progress made by families on barriers that 
...
temp1 <- createTableImproved(7, "15", 4)
...
for(j in 1:length(final$TCode)) final$food_aggregateScorePos[j] <- method1improved(j, "15", 4)

summary(as.factor(final$food_aggregateScorePos))
plot((as.factor(final$food_aggregateScorePos)))

progressed <- final$TCode[final$food_aggregateScorePos!=0]
##################### Deteriorated
...
temp1 <- createTableWorsened(7, "15", 4)
...
for(j in 1:length(final$TCode)) final$food_aggregateScoreNeg[j] <- method1worsened(j, "15", 4)

summary(as.factor(final$food_aggregateScoreNeg))
plot((as.factor(final$food_aggregateScoreNeg)))

regressed <- final$TCode[final$food_aggregateScoreNeg!=0]
...
################################################################################3
################################################################################
##### As a variation of Method 2, another index would look for each Q10 subquestion whether in 2015 a family was in a very or somewhat difficult situation, aggregate the results for the food domain in 2015 and also in 2017 and then see for each family if they have a higher, lower, or same aggregate “very or somewhat difficult” food score in 2017 compared with 2015.
##### METHOD 2: Count of barriers in 2015 minus count of barriers in 2017 for each TCode.
################################################################################

method2food <- function(j, ques, number_of_subques) {
  include <- 1 
  x <- createTableImproved(j, ques, number_of_subques)
  count <- 0
...
temp1 <- method2food(5, "15", 4)
final$AggregatefoodMethod2 <- NA
for(i in 1:length(final$TCode)) if(method2food(i, "15", 4)[3]==1) final$AggregatefoodMethod2[i] <- method2food(i, "15", 4)[2] - method2food(i, "15", 4)[1]

summary(as.factor(final$AggregatefoodMethod2))
final$temp[final$AggregatefoodMethod2>0] <- "Positive"
final$temp[final$AggregatefoodMethod2==0] <- "Neutral"
final$temp[final$AggregatefoodMethod2<0] <- "Negative"
...
summary(final$Q36_4_15)
summary(final$Q36_4_17)
final$temp <- NA
for(i in 1:length(final$TCode)) {
  if(!is.na(final$Q36_4_15[i]) & final$Q36_4_15[i] %in% c("1. Urgent situation, currently in crisis", "2. Vulnerable, need support to move forward")) {
    if(!is.na(final$Q36_4_15[i]) & !is.na(final$Q36_4_17[i]) & as.integer(final$Q36_4_17[i]) > as.integer(final$Q36_4_15[i])) final$temp[i] <- "Positive" 
  }
}
for(i in 1:length(final$TCode)) {
  if(!is.na(final$Q36_4_17[i]) & final$Q36_4_17[i] %in% c("1. Urgent situation, currently in crisis", "2. Vulnerable, need support to move forward")) {
    if(!is.na(final$Q36_4_15[i]) & !is.na(final$Q36_4_17[i]) & as.integer(final$Q36_4_17[i]) < as.integer(final$Q36_4_15[i])) final$temp[i] <- "Negative" 
  }
}
