...
#######################################################################################################
## this index would measure for each subquestion of (only) housing domain, if a family moved between 2015 and 2017 from “very difficult “ to something 
## better (including “somewhat difficult”) or from “somewhat difficult” to something better.  Then add up the results for all 
## subquestions for the housing domain. This variation of Method 1 would indicate progress made by families on barriers that 
...
temp1 <- createTableImproved(7, "3", 9)
...
for(j in 1:length(final$TCode)) final$housing_aggregateScorePos[j] <- method1improved(j, "3", 9)

summary(as.factor(final$housing_aggregateScorePos))
plot((as.factor(final$housing_aggregateScorePos)))

progressed <- final$TCode[final$housing_aggregateScorePos!=0]
##################### Deteriorated
...
temp1 <- createTableWorsened(7, "3", 9)
...
for(j in 1:length(final$TCode)) final$housing_aggregateScoreNeg[j] <- method1worsened(j, "3", 9)

summary(as.factor(final$housing_aggregateScoreNeg))
plot((as.factor(final$housing_aggregateScoreNeg)))

regressed <- final$TCode[final$housing_aggregateScoreNeg!=0]
...
################################################################################3
################################################################################
##### As a variation of Method 2, another index would look for each Q10 subquestion whether in 2015 a family was in a very or somewhat difficult situation, aggregate the results for the housing domain in 2015 and also in 2017 and then see for each family if they have a higher, lower, or same aggregate “very or somewhat difficult” housing score in 2017 compared with 2015.
##### METHOD 2: Count of barriers in 2015 minus count of barriers in 2017 for each TCode.
################################################################################

method2housing <- function(j, ques, number_of_subques) {
  include <- 1 
  x <- createTableImproved(j, ques, number_of_subques)
  count <- 0
...
temp1 <- method2housing(5, "3", 9)
final$AggregatehousingMethod2 <- NA
for(i in 1:length(final$TCode)) if(method2housing(i, "3", 9)[3]==1) final$AggregatehousingMethod2[i] <- method2housing(i, "3", 9)[2] - method2housing(i, "3", 9)[1]

summary(as.factor(final$AggregatehousingMethod2))
final$temp[final$AggregatehousingMethod2>0] <- "Positive"
final$temp[final$AggregatehousingMethod2==0] <- "Neutral"
final$temp[final$AggregatehousingMethod2<0] <- "Negative"
...
summary(final$Q36_1_15)
summary(final$Q36_1_17)
final$temp <- NA
for(i in 1:length(final$TCode)) {
  if(!is.na(final$Q36_1_15[i]) & final$Q36_1_15[i] %in% c("1. Urgent situation, currently in crisis", "2. Vulnerable, need support to move forward")) {
    if(!is.na(final$Q36_1_15[i]) & !is.na(final$Q36_1_17[i]) & as.integer(final$Q36_1_17[i]) > as.integer(final$Q36_1_15[i])) final$temp[i] <- "Positive" 
  }
}
for(i in 1:length(final$TCode)) {
  if(!is.na(final$Q36_1_17[i]) & final$Q36_1_17[i] %in% c("1. Urgent situation, currently in crisis", "2. Vulnerable, need support to move forward")) {
    if(!is.na(final$Q36_1_15[i]) & !is.na(final$Q36_1_17[i]) & as.integer(final$Q36_1_17[i]) < as.integer(final$Q36_1_15[i])) final$temp[i] <- "Negative" 
  }
}
