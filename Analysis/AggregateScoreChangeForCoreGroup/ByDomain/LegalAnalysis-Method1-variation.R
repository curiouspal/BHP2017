...
temp1 <- createTableWorsened(7, "27", 3)
...
summary(final$Q36_8_15)
summary(final$Q36_8_17)
final$temp <- NA
for(i in 1:length(final$TCode)) {
  if(!is.na(final$Q36_8_15[i]) & final$Q36_8_15[i] %in% c("1. Urgent situation, currently in crisis", "2. Vulnerable, need support to move forward")) {
    if(!is.na(final$Q36_8_15[i]) & !is.na(final$Q36_8_17[i]) & as.integer(final$Q36_8_17[i]) > as.integer(final$Q36_8_15[i])) final$temp[i] <- "Positive" 
  }
}
for(i in 1:length(final$TCode)) {
  if(!is.na(final$Q36_8_17[i]) & final$Q36_8_17[i] %in% c("1. Urgent situation, currently in crisis", "2. Vulnerable, need support to move forward")) {
    if(!is.na(final$Q36_8_15[i]) & !is.na(final$Q36_8_17[i]) & as.integer(final$Q36_8_17[i]) < as.integer(final$Q36_8_15[i])) final$temp[i] <- "Negative" 
  }
}
