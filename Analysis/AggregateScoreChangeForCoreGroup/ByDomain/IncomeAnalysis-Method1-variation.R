...
#######################################################################################################
## this index would measure for each subquestion of (only) income domain, if a family moved between 2015 and 2017 from “very difficult “ to something 
## better (including “somewhat difficult”) or from “somewhat difficult” to something better.  Then add up the results for all 
## subquestions for the income domain. This variation of Method 1 would indicate progress made by families on barriers that 
...
createTableImproved <- function(j, ques, number_of_subques) {
  ...
  temp$diff <- 0
  temp$diff[as.integer(temp$y15) %in% c(1,2) & (temp$y15) < (temp$y17)] <- 1
  print(temp)
  return(temp)
}
...
for(j in 1:length(final$TCode)) final$income_aggregateScorePos[j] <- method1improved(j, "10", 8)

summary(as.factor(final$income_aggregateScorePos))
plot((as.factor(final$income_aggregateScorePos)))

progressed <- final$TCode[final$income_aggregateScorePos!=0]
##################### Deteriorated
...
createTableWorsened <- function(j, ques, number_of_subques) {
  ...
  temp$diff <- 0
  temp$diff[as.integer(temp$y15) %in% c(3,4,5) & (temp$y15) > (temp$y17)] <- 1
  print(temp)
  return(temp)
}
...
for(j in 1:length(final$TCode)) final$income_aggregateScoreNeg[j] <- method1worsened(j, "10", 8)

summary(as.factor(final$income_aggregateScoreNeg))
plot((as.factor(final$income_aggregateScoreNeg)))

regressed <- final$TCode[final$income_aggregateScoreNeg!=0]
...
################################################################################3
################################################################################
##### As a variation of Method 2, another index would look for each Q10 subquestion whether in 2015 a family was in a very or somewhat difficult situation, aggregate the results for the income domain in 2015 and also in 2017 and then see for each family if they have a higher, lower, or same aggregate “very or somewhat difficult” income score in 2017 compared with 2015.
...
temp1 <- method2income(5, "10", 8)
final$AggregateIncomeMethod2 <- NA
for(i in 1:length(final$TCode)) if(method2income(i, "10", 8)[3]==1) final$AggregateIncomeMethod2[i] <- method2income(i, "10", 8)[2] - method2income(i, "10", 8)[1]

summary(as.factor(final$AggregateIncomeMethod2))
final$temp[final$AggregateIncomeMethod2>0] <- "Positive"
final$temp[final$AggregateIncomeMethod2==0] <- "Neutral"
final$temp[final$AggregateIncomeMethod2<0] <- "Negative"
