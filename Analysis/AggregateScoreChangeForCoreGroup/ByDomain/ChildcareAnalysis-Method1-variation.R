...
#######################################################################################################
## this index would measure for each subquestion of (only) childcare domain, if a family moved between 2015 and 2017 from “very difficult “ to something 
## better (including “somewhat difficult”) or from “somewhat difficult” to something better.  Then add up the results for all 
## subquestions for the childcare domain. This variation of Method 1 would indicate progress made by families on barriers that 
...
createTableImproved <- function(j, ques, number_of_subques) {
  ...
  temp$diff <- 0
  temp$diff[as.integer(temp$y15) %in% c(4, 5) & (temp$y15) > (temp$y17)] <- 1
  print(temp)
  return(temp)
}

temp1 <- createTableImproved(7, "22", 3)
...
for(j in 1:length(final$TCode)) final$childcare_aggregateScorePos[j] <- method1improved(j, "22", 3)

summary(as.factor(final$childcare_aggregateScorePos))
plot((as.factor(final$childcare_aggregateScorePos)))

progressed <- final$TCode[final$childcare_aggregateScorePos!=0]
##################### Deteriorated
...
createTableWorsened <- function(j, ques, number_of_subques) {
  ...
  temp$diff <- 0
  temp$diff[as.integer(temp$y15) %in% c(1,2,3) & (temp$y15) < (temp$y17)] <- 1
  print(temp)
  return(temp)
}

temp1 <- createTable(7, "22", 3)
...
for(j in 1:length(final$TCode)) final$childcare_aggregateScoreNeg[j] <- method1worsened(j, "22", 3)

summary(as.factor(final$childcare_aggregateScoreNeg))
plot((as.factor(final$childcare_aggregateScoreNeg)))

regressed <- final$TCode[final$childcare_aggregateScoreNeg!=0]
...
################################################################################3
################################################################################
##### As a variation of Method 2, another index would look for each Q10 subquestion whether in 2015 a family was in a very or somewhat difficult situation, aggregate the results for the childcare domain in 2015 and also in 2017 and then see for each family if they have a higher, lower, or same aggregate “very or somewhat difficult” childcare score in 2017 compared with 2015.
...
temp1 <- method2childcare(5, "22", 3)
final$AggregatechildcareMethod2 <- NA
for(i in 1:length(final$TCode)) if(method2childcare(i, "22", 3)[3]==1) final$AggregatechildcareMethod2[i] <- method2childcare(i, "22", 3)[2] - method2childcare(i, "22", 3)[1]

summary(as.factor(final$AggregatechildcareMethod2))
final$temp[final$AggregatechildcareMethod2>0] <- "Positive"
final$temp[final$AggregatechildcareMethod2==0] <- "Neutral"
final$temp[final$AggregatechildcareMethod2<0] <- "Negative"
