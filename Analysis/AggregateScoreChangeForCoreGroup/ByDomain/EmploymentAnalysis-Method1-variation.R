...
#######################################################################################################
## this index would measure for each subquestion of (only) employment domain, if a family moved between 2015 and 2017 from “very difficult “ to something 
## better (including “somewhat difficult”) or from “somewhat difficult” to something better.  Then add up the results for all 
## subquestions for the employment domain. This variation of Method 1 would indicate progress made by families on barriers that 
...
temp1 <- createTableImproved(7, "8", 14)
...
for(j in 1:length(final$TCode)) final$employment_aggregateScorePos[j] <- method1improved(j, "8", 14)

summary(as.factor(final$employment_aggregateScorePos))
plot((as.factor(final$employment_aggregateScorePos)))

progressed <- final$TCode[final$employment_aggregateScorePos!=0]
##################### Deteriorated
...
for(j in 1:length(final$TCode)) final$employment_aggregateScoreNeg[j] <- method1worsened(j, "8", 14)

summary(as.factor(final$employment_aggregateScoreNeg))
plot((as.factor(final$employment_aggregateScoreNeg)))

regressed <- final$TCode[final$employment_aggregateScoreNeg!=0]
...
################################################################################3
################################################################################
##### As a variation of Method 2, another index would look for each Q10 subquestion whether in 2015 a family was in a very or somewhat difficult situation, aggregate the results for the employment domain in 2015 and also in 2017 and then see for each family if they have a higher, lower, or same aggregate “very or somewhat difficult” employment score in 2017 compared with 2015.
...
temp1 <- method2employment(5, "8", 14)
final$AggregateemploymentMethod2 <- NA
for(i in 1:length(final$TCode)) if(method2employment(i, "8", 14)[3]==1) final$AggregateemploymentMethod2[i] <- method2employment(i, "8", 14)[2] - method2employment(i, "8", 14)[1]

summary(as.factor(final$AggregateemploymentMethod2))
final$temp[final$AggregateemploymentMethod2>0] <- "Positive"
final$temp[final$AggregateemploymentMethod2==0] <- "Neutral"
final$temp[final$AggregateemploymentMethod2<0] <- "Negative"
