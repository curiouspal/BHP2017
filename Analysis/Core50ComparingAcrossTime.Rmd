---
title: "Comparing Core group of 50 PH households across time"
...
core50 <- (intersect(intersect(ph2015, ph2017), ph2014))
final <- final[final$TCode %in% core50, ]
```
...
percentSatisfied <- function(qn, i) {
  ...
  vg_14 <- sum(as.integer(summary(as.factor(a14))[c("c. Neutral", "d. Poor", "e. Very poor")]))
  g_14 <- sum(as.integer(summary(as.factor(a14))[c("a. Very good", "b. Good")]))
  tot_14 <- length(a14) - length(a14[is.na(a14)])
  percent_y_14 <- (g_14)* 100 / (vg_14 + g_14)
...
ggplot(d, aes(reorder(Question, Percent_Satisfied), Percent_Satisfied, group=Year)) + geom_bar(stat = "identity", width = 0.7, position="dodge", aes(fill=Year), alpha=1) +
  ...
  ggtitle("\nEvaluate Childcare?\n") +
  #scale_fill_discrete(name = "", labels = c("Y in '14, N in '15", "No change", "N in '14, Y in '15")) +
  geom_text(aes(y = Percent_Satisfied, label=Percent_Yes_text), stat="identity", position=position_dodge(width = 0.9), hjust="inward", vjust=0.3, size=4, alpha=1) +
  scale_fill_manual(values = c("aquamarine2", "aquamarine3", "aquamarine4"), name="")
